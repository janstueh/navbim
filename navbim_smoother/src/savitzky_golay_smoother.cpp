...
{
  throw nav2_core::SmootherException(
    "SavitzkyGolaySmoother::smooth(path, max_time) is not supported. "
  ...
  const rclcpp::Duration & max_time)
{
  ...
  for (unsigned int i = 0; i != path_segments.size(); i++) {
    if (path_segments[i].end - path_segments[i].start > minimum_points) {
      ...
      // Smooth path segment
      success = success && smoothImpl(curr_path_segment, reversing_segment, costmap);
  ...
  const nav2_costmap_2d::Costmap2D * costmap)
{
  ...
  // Store original path in case we need to revert
  ...
  // Check for collisions after initial smoothing
  if (costmap) {
    for (unsigned int i = 1; i < path_size - 1; i++) {
      ...
      if (costmap->worldToMap(
            ...
            mx, my))
      {
        float cost = static_cast<float>(costmap->getCost(mx, my));
        if (cost > nav2_costmap_2d::MAX_NON_OBSTACLE && cost != nav2_costmap_2d::NO_INFORMATION) {
          RCLCPP_DEBUG(
            logger_,
            "SavitzkyGolay smoothing resulted in collision at pose %u. Reverting to original path.",
            i);
          path = original_path;
  ...
  if (do_refinement_) {
    for (int i = 0; i < refinement_num_; i++) {
      ...
      // Check for collisions after each refinement pass
      if (costmap) {
        for (unsigned int idx = 1; idx < path_size - 1; idx++) {
          unsigned int mx, my;
          if (costmap->worldToMap(
                ...
                mx, my))
          {
            ...
            if (cost > nav2_costmap_2d::MAX_NON_OBSTACLE && cost != nav2_costmap_2d::NO_INFORMATION) {
              RCLCPP_DEBUG(
                logger_,
                "SavitzkyGolay refinement %d resulted in collision at pose %u. Reverting to original path.",
                i, idx);
              path = original_path;
              smoother_utils::updateApproximatePathOrientations(path, reversing_segment);
              return false;
...
PLUGINLIB_EXPORT_CLASS(navbim_smoother::SavitzkyGolaySmoother, nav2_core::Smoother)
