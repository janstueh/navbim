#ifndef NAVBIM_TOPOMAP_SERVER__TOPOLOGICAL_MAP_TYPES_HPP_
#define NAVBIM_TOPOMAP_SERVER__TOPOLOGICAL_MAP_TYPES_HPP_
 ...
 * stair) use different subsets of these properties.
 */
struct NodeProperties {
  int id;                                    // Node ID (required)
  std::string name;                          // Node name (optional)
  std::string type;                          // Node type: "floor", "room", "transition", "stair"
...
struct EdgeProperties {
  ...
  std::shared_ptr<std::vector<std::array<double, 3>>> path;  // [x, y, z] waypoints
  std::string path_frame_id{"ifc"};          // Coordinate frame of the path waypoints
 ...
 */
using TopologicalMap = boost::adjacency_list<
...
using Vertex = TopologicalMap::vertex_descriptor;
using Edge = TopologicalMap::edge_descriptor;
using VertexIterator = TopologicalMap::vertex_iterator;
using EdgeIterator = TopologicalMap::edge_iterator;

}  // namespace navbim_topomap_server

#endif  // NAVBIM_TOPOMAP_SERVER__TOPOLOGICAL_MAP_TYPES_HPP_
